...
export function AdminLoading() {
  return (
    <AdminShell>
      ...
      <section className={`${panelClass} mt-6`}>
        <SectionHeading eyebrow="Karşılaştırma" title="Kim ne kadar aldı, kim ne kadar harcadı?" description="“Bölüşüldü” kayıtları üyeler arasında eşit paylaştırılır. Eşitlik hesabı, alınan para eksi harcanan para üzerinden yapılır." right={<div className="rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-700">Eşitle: tümünü Bölüşüldü yap</div>} />
    ...
    <AdminShell maxWidth="max-w-4xl">
      ...
      <section className={panelClass}>
        <div className="space-y-7">
          ...
          <section className="border-t border-slate-100 pt-6"><SectionHeading eyebrow="İletişim kartları" title="Butonlar" description="Butonları açıp kapatın, özel bağlantılar ekleyin ve sıralarını belirleyin." /><div className="grid gap-3"><div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><p className="text-sm font-black text-slate-800">Buton sırası</p><p className="mt-1 text-xs leading-5 text-slate-400">Yukarı ve aşağı oklarıyla profil sayfasındaki görünüm sırasını değiştirin.</p><div className="mt-3 grid gap-2">{["Ara", "WhatsApp", "Telegram", "Mail", "LinkedIn", "Kişilere Ekle", "Konum", "Instagram", "IBAN"].map((label) => <div key={label} className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2"><span className="min-w-0 flex-1 truncate text-sm font-bold text-slate-700">{label}</span><span className="rounded-lg border border-slate-200 px-2 py-1 text-sm font-bold text-slate-500">↑</span><span className="rounded-lg border border-slate-200 px-2 py-1 text-sm font-bold text-slate-500">↓</span></div>)}</div></div><StaticProfileSetting label="Ara" placeholder="+90 5xx xxx xx xx" /><StaticProfileSetting label="WhatsApp" placeholder="905xx xxx xx xx" /><StaticProfileSetting label="Telegram" placeholder="https://t.me/kullanici" /><StaticProfileSetting label="Mail" placeholder="isim@firma.com" /><StaticProfileSetting label="LinkedIn" placeholder="https://linkedin.com/in/..." /><StaticProfileSetting label="Instagram" placeholder="https://instagram.com/..." /><StaticProfileSetting label="Konum" placeholder="https://maps.google.com/..." /><StaticProfileSetting label="Kişilere Ekle" checks /><div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm font-black text-slate-800">IBAN</p><StaticCheck>Göster</StaticCheck></div><StaticSettingInput placeholder="TR00 0000 0000 0000 0000 0000 00" className="mt-3 uppercase" /><p className="mt-2 text-xs leading-5 text-slate-400">Profilde mobil uyumlu bir kopyalama kartı olarak gösterilir.</p></div><div className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-black text-slate-800">Özel butonlar</p><p className="mt-1 text-xs leading-5 text-slate-400">Kendi başlığınız ve bağlantınızla yeni butonlar ekleyin.</p></div><StaticButton variant="compact">+ Özel buton ekle</StaticButton></div><div className="mt-4 grid gap-3"><div className="rounded-2xl border border-slate-200 bg-white p-3"><div className="grid gap-3 sm:grid-cols-2"><StaticField label="Buton adı" placeholder="Web sitesi" /><StaticField label="Bağlantı" placeholder="https://..." /></div></div></div></div></div></section>
